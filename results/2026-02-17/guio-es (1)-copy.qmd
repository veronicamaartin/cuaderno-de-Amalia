...
```
#como los paquetes los tengo instalados en bin, se pone ~/bin/datasets y lo mismo con dataformats
...
#~/bin/dataformat tsv genome --help en el terminal 
...
kable(genomas) 
```
#da error, que no hay en la libreria un paquete llamado kableExtra,
#poniendo el comando system.file(package = "kableExtra") para ver si esta nos dice que no esta instalado, lo instalamos, no se puede instalar, en vez de library(kableExtra), 
#ponemos libray(knitr) y eliminamos desde el primer %, ahora ya va, da como resultado una tabla
ls()
class(genomas)
...
#con estos comandos podemos identificar las lineas repetidas en un marco de datos, 
#en concreto en el objeto genomas, y con la función sum nos da el número de líneas repetidas
:::
...
kable(genomas) 
```
#como antes, he tenido que borrar desde los porcentajes en la linea kable(genomas) y poner 
#library(knitr) para decirle de donde sacamos la función kable
...
#file.choose() para saber la ruta completa del archivo data_report.json1
#r nos devuelve la dirección completa, la copiamos dentro de readLines
```{r}
#| message: false
...
#para saber las columnas invariables
