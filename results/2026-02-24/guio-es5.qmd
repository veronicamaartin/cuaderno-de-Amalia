...
curl_download('https://www.ebi.ac.uk/ena/portal/api/search?result=assembly&query=tax_tree(6157) AND genome_representation="full"&fields=version,tax_id,scientific_name,last_updated,base_count&limit=0',
              destfile = 'z1.txt')
```
#da error, pone que la URL esta usando un formato malo o ilegal o que falta la URL, se cambia el texto de la direccion web al ejecutarlo, 
#la unica diferencia que hay con el siguiente codigo (que si que da bien), es que dentro de curl_download utiliza tambien URLencode. El comando URLencode se utiliza para ara codificar caracteres especiales en cadenas de texto para que sean seguros al usarse en URLs,  convirtiendo espacios en %20 y caracteres reservados en sus equivalentes hexadecimales.
```{r}
curl_download(
