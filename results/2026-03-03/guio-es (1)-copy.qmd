...
install.packages("kableExtra") #no nos deja instalar el kableExtra, como ya pasó el otro dia, sustituimos el kableExtra por el paquete knitr y borramos el porcentaje
```{r}
library('knitr')
...
#Cambiamos el bloque anterior poniendo lo que nos pide el ejercicio. Como se trata de una proteina de Swiss-Prot ponemos reviewed y para extraer la secuencia ponemos sequence
```{r}
...
https://www.ebi.ac.uk/interprot/api/protein/taxonomy
https://www.ebi.ac.uk/interpro/api/protein
#creamos dos objetos con las dos urls
```{r}
taxa <- fromJSON(paste0(API, 'taxonomy/protein'))
taxonomia <- fromJSON(paste0(API, 'taxonomy'))
str(taxa)
str(taxonomia)
todotaxonomia <- taxonomia[['taxa']][['uniprot']]
solotaxones <- taxa [['taxa']][['uniprot']]
todotaxonomia
solotaxones
todotaxonomia - solotaxones
```
#para obtener los taxones que no tienen ninguna proteina asignada: 
todo - solotaxones
#ahora para obtener la proporcion de las que tienen estructuras terciarias
https://www.ebi.ac.uk/interprot/api/protein/structure
https://www.ebi.ac.uk/interprot/api/protein
```{r}
estructura <- fromJSON(paste0(API, 'protein/structure'))
todo <- fromJSON(paste0(API, 'protein'))
str(estructura)
str(todo)
estructura3 <- estructura[['structures']][['pdb']]
todas <- todo[['proteins']][['uniprot']]
estructura3
(estructura3/todas)*100
